  ...
  return (
    <>
      <motion.aside
        ...
        className="hidden lg:flex relative h-full border-l border-border bg-background overflow-hidden flex-shrink-0"
      >
          ...
          >
            ...
            >
              <div className="flex items-center justify-between px-4 h-12 border-b border-border">
                <span className="text-sm font-semibold text-text-primary">Parameters</span>
                <div className="flex items-center gap-1">
                  <button onClick={handleReset} className="p-1.5 rounded-md text-text-muted hover:text-text-primary hover:bg-white/[0.04] transition-all" title="Reset"><RotateCcw className="w-3.5 h-3.5" /></button>
                  <button onClick={onToggle} className="p-1.5 rounded-md text-text-muted hover:text-text-primary hover:bg-white/[0.04] transition-all"><X className="w-3.5 h-3.5" /></button>
                </div>
              </div>
              <div className="overflow-y-auto p-3 space-y-3 pb-6" style={{ scrollbarWidth: 'thin', maxHeight: 'calc(80vh - 48px)' }}>
                {params.map(p => (
                  <ParameterCard key={p.id} name={p.name} description={p.desc} value={getValue(p.id)} min={p.min} max={p.max} step={p.step} decimals={p.decimals} onChange={v => onUpdate(p.id, v)} learnMore={p.info} />
                ))}
                <div className="rounded-xl bg-card border border-border overflow-hidden">
                  <button onClick={() => setSystemExpanded(!systemExpanded)} className="w-full flex items-center justify-between px-3 py-3 text-sm font-medium text-text-primary hover:bg-white/[0.02] transition-all">
                    <div className="flex items-center gap-2"><FileText className="w-4 h-4 text-text-muted" />System Prompt</div>
                    <ChevronDown className={'w-4 h-4 text-text-muted transition-transform ' + (systemExpanded ? 'rotate-180' : '')} />
                  </button>
                  <AnimatePresence>{systemExpanded && (
                    <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                      <div className="px-3 pb-3 space-y-2">
                        <div className="flex gap-1.5 flex-wrap">
                          {['Teacher', 'Professor', 'Code'].map(p => (
                            <button key={p} onClick={() => { const presets: Record<string, string> = { Teacher: 'You are a friendly, patient teacher. Explain clearly and stay helpful and respectful.', Professor: 'You are a formal university professor. Be precise and direct.', Code: 'Answer ONLY programming and coding questions.' }; onUpdate('system_prompt', presets[p]) }} className="px-2.5 py-1 rounded-md text-xs font-medium bg-surface border border-border text-text-muted hover:border-white/20 hover:text-text-primary transition-all">{p}</button>
                          ))}
                        </div>
                        <textarea value={options.system_prompt} onChange={e => onUpdate('system_prompt', e.target.value)} rows={3} className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-xs text-text-primary placeholder:text-text-muted resize-none focus:outline-none focus:border-white/20 transition-colors" placeholder="Set the behavior of the AI..." />
                      </div>
                    </motion.div>
                  )}</AnimatePresence>
                </div>
